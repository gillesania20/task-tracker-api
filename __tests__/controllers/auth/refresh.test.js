...
describe('POST api/auth/refresh', () => {
    ...
    test('refresh token not available', async () => {
        ...
        expect(res.json).toHaveBeenCalledWith({
            message: 'no refresh token', accessToken: null, userId: null
        });
    });
    test('cannot find user', async () => {
        const REFRESH_TOKEN = process.env.REFRESH_TOKEN;
        const STATIC_PROJECTION = '_id username role';
        const req = {
            cookies: {
                jwt: 'jwtCookie'
            }
        ...
        expect(res.status).toHaveBeenCalledWith(404);
        expect(res.json).toHaveBeenCalledWith({
            message: 'user not found', accessToken: null, userId: null});
    ...
    test('successfully refreshed the accessToken', async () => {
        ...
        jwt.verify.mockReturnValue({ username: 'username', role: 'role'});
        userFindOne.mockResolvedValue({ _id: '_id_1', username: 'username2', role: 'role2'});
